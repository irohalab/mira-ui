...
export type FullScreenAPIProperty = {
  enabled: string,
  element: string,
  request: string,
  exit: string,
...
export const BROWSER_FULLSCREEN_API: {[key: string]: FullScreenAPIProperty} = {
    w3: {
...
export class FullScreenAPI {
    ...
    constructor(private _mediaElement: HTMLMediaElement, private _hostElement: Element) {
        ...
        observableFromEvent(fsElemDispatcher, this.polyfill.onchange).subscribe(() => {
            this.isFullscreen = !!(document as any)[ this.polyfill.element ];
    ...
    exit() {
        ...
        if (this.isAvailable && this.nativeFullscreen) {
            (document as any)[ this.polyfill.exit ]();
        }
    }
}
