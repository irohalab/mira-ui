...
export type KeyMapDict = {[key: string]: number};
...
export class VideoPlayerShortcuts {
    ...
    constructor(private _hostElement: Element, videoPlayer: VideoPlayer, captureService: VideoCapture) {
        ...
        this._subscription.add(
            observableFromEvent<KeyboardEvent>(_hostElement, 'keydown').pipe(
                filter(() => this.focus),
                filter((event: KeyboardEvent) => {
                    return Object.keys(KEY_MAP)
                        .map(name => (KEY_MAP as KeyMapDict)[name])
        ...
        this._subscription.add(
            observableFromEvent<KeyboardEvent>(_hostElement, 'keyup').pipe(
                filter(() => this.focus),
                filter((event: KeyboardEvent) => {
                    return Object.keys(KEY_MAP)
                        .map(name => (KEY_MAP as KeyMapDict)[name])
